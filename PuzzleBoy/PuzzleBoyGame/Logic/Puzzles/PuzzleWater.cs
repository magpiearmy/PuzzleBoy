...
namespace PuzzleBoy
{
    class PuzzleWater : Puzzle
    {
        const float waterFlowDelay = 3500;
        float waterFlowSpeed;
        float timeSinceWaterMoved;
        int waterFlowDirection;
        bool waterFlowing;
        Vector2 currentWaterPos;

        public PuzzleWater(IServiceProvider serviceProvider, Stream fileStream, Viewport viewport, int puzzleId)
        {
        ...
        {
            backgroundTexture = Content.Load<Texture2D>("Puzzles/Pieces/WaterBackground");
            selectorTexture = Content.Load<Texture2D>("Puzzles/Pieces/Selector");

            pHorizTexture = Content.Load<Texture2D>("Puzzles/Pieces/PipeH");
            pVertTexture = Content.Load<Texture2D>("Puzzles/Pieces/PipeV");
            pTopLeftTexture = Content.Load<Texture2D>("Puzzles/Pieces/PipeTL");
            pTopRightTexture = Content.Load<Texture2D>("Puzzles/Pieces/PipeTR");
            pBottomLeftTexture = Content.Load<Texture2D>("Puzzles/Pieces/PipeBL");
            pBottomRightTexture = Content.Load<Texture2D>("Puzzles/Pieces/PipeBR");
        ...
        protected void UpdateFlow(GameTime gameTime)
        {
            if (!waterFlowing)
            {
                ...
                if (timeSinceStart.TotalMilliseconds >= waterFlowDelay)
                {
                    BeginFlow();
                }
            }
            ...
            {
                timeSinceWaterMoved += (float)gameTime.ElapsedGameTime.TotalMilliseconds;

                float timeForWaterMovement = PuzzlePiece.Width / waterFlowSpeed;
                if (timeSinceWaterMoved >= timeForWaterMovement * 1000)
                {
                    currentWaterPos = GetNextTilePos(waterFlowDirection, currentWaterPos);
                    if (currentWaterPos == endTile)
                    {
                        waterFlowing = false;
                    ...
                    {
                        if (currentWaterPos.X < 0 || currentWaterPos.X > Width - 1 || currentWaterPos.Y < 0 || currentWaterPos.Y > Height - 1)
                        {
                            waterFlowDirection = 0;
                        }
                        else
                        {
                            waterFlowDirection = _pieces[(int)currentWaterPos.X, (int)currentWaterPos.Y].GetDirection(waterFlowDirection);
                        }
                    }
                    timeSinceWaterMoved = 0.0f;
                }

                // Check if the water flow is still valid
                if (waterFlowDirection == 0)
                {
                    _state = PuzzleState.eFailed;
                }
                else
                {
                    _pieces[(int)currentWaterPos.X, (int)currentWaterPos.Y].Fill(Color.Aqua);
                }
            }
        }
